...
export default function OrderDeliveryDetails({ order, isBg }: { order: OrderDelivery; isBg: boolean }) {
  return <div className="mt-4 grid gap-4 rounded-xl bg-slate-50 p-4 text-sm sm:grid-cols-2">
    ...
    <div><h3 className="font-semibold">{isBg ? 'Плащане' : 'Payment'}</h3><p>{order.paymentMethod === 'bank-transfer' ? (isBg ? 'Банков превод' : 'Bank transfer') : order.paymentMethod === 'cash-on-delivery' ? (isBg ? 'В брой при доставка' : 'Cash on delivery') : (isBg ? 'Не е указан' : 'Not specified')}</p></div>
