...
const menuItems = [
  { to: "/promotions", labelBg: "Промоции", labelEn: "Promotions", badgeBg: "Акция", badgeEn: "Sale", icon: TagIcon },
  { to: "/new-products", labelBg: "Нови стоки", labelEn: "New products", badgeBg: "Ново", badgeEn: "New", icon: SparklesIcon },
  { to: "/best-sellers", labelBg: "Най-продавани", labelEn: "Best sellers", badgeBg: "ТОП", badgeEn: "TOP", icon: SparklesIcon },
  { to: "/brands", labelBg: "По марка", labelEn: "By brand", badgeBg: null, badgeEn: null, icon: null },
  { to: "/contact", labelBg: "Контакти", labelEn: "Contact", badgeBg: null, badgeEn: null, icon: PhoneIcon },
];
...
const Navbar=()=>{
 ...
 useEffect(()=>{(async()=>{try{const r=await fetch(`${process.env.NEXT_PUBLIC_API_URL}/Categories`);setCategories(r.ok?normalizeCategories(await r.json()):[]);}catch{setCategories([]);}})();},[]);
 ...
 return <header className="sticky top-0 z-40 bg-white shadow-sm dark:bg-black"><div className="site-container flex items-center gap-3 py-3"><button onClick={()=>setIsMenuOpen(v=>!v)} className="inline-flex h-11 w-11 items-center justify-center border xl:hidden">{isMenuOpen?<XMarkIcon className="h-5 w-5"/>:<Bars3Icon className="h-5 w-5"/>}</button><Link to="/"><img src="/higiqlogo.png" alt="HygiaTrade" className="h-12 w-auto"/></Link><form onSubmit={handleSearchSubmit} className="hidden flex-1 xl:block"><div className="relative mx-auto max-w-2xl"><input type="search" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder={t("nav.searchPlaceholder")} className="h-12 w-full border bg-[#f3f3f3] px-4 pr-14"/><button className="absolute right-0 top-0 flex h-12 w-14 items-center justify-center bg-[#18b99f] text-white"><MagnifyingGlassIcon className="h-6 w-6"/></button></div></form><div className="hidden shrink-0 items-center gap-3 2xl:flex"><PhoneIcon className="h-8 w-8 text-[#70808d]"/><div><p className="text-sm font-bold text-[#7a8791]">{CONTACT_PHONE}</p><p className="text-xs font-bold uppercase text-[#263b4d]">{isBg?"Свържете се с нас":"Contact us"}</p><p className="text-[11px] text-[#7a8791]">{CONTACT_EMAIL}</p></div></div><div className="ml-auto hidden items-center gap-2 xl:flex">{actions}</div><div className="ml-auto xl:hidden"><Link to="/cart"><ShoppingBagIcon className="h-7 w-7"/></Link></div></div><div className="hidden bg-[#263b4d] text-white xl:block"><div className="site-container flex items-center"><div className="group relative"><button onClick={()=>navigate("/products")} className="flex h-12 items-center gap-2 bg-[#18b99f] px-5 font-bold"><Bars3Icon className="h-6 w-6"/>{isBg?"Категории":"Categories"}</button><div className="invisible absolute left-0 top-full z-50 w-64 bg-white p-2 text-black opacity-0 group-hover:visible group-hover:opacity-100">{categories.map(c=><button key={c.id} onClick={()=>navigate(`/products?category=${encodeURIComponent(c.id)}`)} className="block w-full px-4 py-2 text-left">{categoryLabel(c)}</button>)}</div></div><nav className="flex flex-1 items-center">{menuItems.map(item=><NavLink key={item.to} to={item.to} className="flex h-12 items-center gap-2 px-5 text-sm font-bold">{item.icon&&<item.icon className="h-4 w-4"/>}{isBg?item.labelBg:item.labelEn}{(isBg?item.badgeBg:item.badgeEn)&&<span className="rounded bg-red-500 px-1 text-[10px]">{isBg?item.badgeBg:item.badgeEn}</span>}</NavLink>)}{isAdmin&&<NavLink to="/admin" className="px-5 text-sm font-bold">Админ</NavLink>}</nav></div></div>{isMenuOpen&&<div className="border-t bg-white p-4 xl:hidden"><form onSubmit={handleSearchSubmit}><input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} className="w-full border p-3" placeholder={t("nav.searchPlaceholder")}/></form><div className="mt-4 grid gap-2">{menuItems.map(i=><Link key={i.to} to={i.to} className="p-2">{isBg?i.labelBg:i.labelEn}</Link>)}</div></div>}</header>;
};
