...
import { getCategoryImage, normalizeCollection } from "../utils/apiResponse";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-3">
        <p className="mb-1 text-[11px] font-bold uppercase tracking-wide text-slate-400 dark:text-white/50">
          {product.categoryName || (language === "bg" ? "Продукт" : "Product")}
        </p>
    ...
    const fetchHomeData = async () => {
      try {
        ...
        if (categoriesResponse.ok) {
          ...
          setCategories(normalizeCollection<Category>(categoriesData));
        }

        if (bestSellersResponse.ok) {
          const bestSellersData = await bestSellersResponse.json();
          setBestSellers(normalizeCollection<Product>(bestSellersData));
        }

        if (latestProductsResponse.ok) {
          const latestProductsData = await latestProductsResponse.json();
          setLatestProducts(normalizeCollection<Product>(latestProductsData));
        }
      } catch {
        setCategories([]);
        setBestSellers([]);
          ...
          {visibleCategories.slice(0, 10).map((category) => (
            ...
            >
              <div className="aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-900">
                <img
                  src={getCategoryImage(category) || "https://images.unsplash.com/photo-1585421514738-01798e348b17?auto=format&fit=crop&w=900&q=80"}
                  alt={category.name}
                  className="h-full w-full object-cover transition duration-500 group-hover:scale-105"
                />
              </div>
