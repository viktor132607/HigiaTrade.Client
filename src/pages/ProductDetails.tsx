...
const ProductDetails = () => {
  ...
  const [selectedPriceTier,setSelectedPriceTier]=useState<"retail"|"wholesale">("retail");
  ...
  useEffect(()=>{if(!product?.id||!product.categoryId){setSimilarProducts([]);setRecentlyViewed([]);return;}let cancelled=false;(async()=>{try{const r=await fetch(`${process.env.NEXT_PUBLIC_API_URL}/Products?PageNumber=1&PageSize=500`);if(!r.ok)return;const d=await r.json();const list:Product[]=Array.isArray(d)?d:Array.isArray(d?.items)?d.items:[];const sameCategory=list.filter(p=>p.id!==product.id&&p.categoryId===product.categoryId);const fallback=list.filter(p=>p.id!==product.id&&p.categoryId!==product.categoryId);if(!cancelled)setSimilarProducts([...sameCategory,...fallback].slice(0,4));try{const stored=JSON.parse(localStorage.getItem(VIEW_HISTORY_KEY)||"[]");const ids:string[]=Array.isArray(stored)?stored.filter((x):x is string=>typeof x==="string"):[];const map=new Map(list.map(p=>[p.id,p]));const history=ids.filter(x=>x!==product.id).map(x=>map.get(x)).filter((x):x is Product=>Boolean(x)).slice(0,4);if(!cancelled)setRecentlyViewed(history);}catch{if(!cancelled)setRecentlyViewed([]);}}catch{if(!cancelled){setSimilarProducts([]);setRecentlyViewed([]);}}})();return()=>{cancelled=true;};},[product?.id,product?.categoryId]);

  useEffect(()=>{
    if(!galleryOpen)return;
    const imageCount=Array.from(new Set([product?.mainImageUrl,...(product?.secondaryImages??[]).map(x=>x.uri)].filter(Boolean))).length;
  ...
  const addToCart=async()=>{if(!product||product.quantity<=0)return;const wholesalePrice=Number(product.wholesalePriceInclVat??product.wholesalePrice??0);const useWholesale=selectedPriceTier==="wholesale"&&wholesalePrice>0;try{dispatch(addItem({id:product.id,title:product.title,regularPrice:useWholesale?wholesalePrice:product.regularPrice,quantity,imageUrl:product.mainImageUrl,mainImageUrl:product.mainImageUrl,discountPercentage:useWholesale?0:product.discountPercentage,discountedPrice:useWholesale?0:product.discountedPrice}));toast.success(tr("Продуктът е добавен в количката.","Product added to cart."));if(token){void fetch(`${process.env.NEXT_PUBLIC_API_URL}/Orders`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({productId:product.id,quantity})}).catch(()=>undefined);}}catch{toast.error(tr("Продуктът не беше добавен в количката.","The product was not added to your cart."));}};
  ...
  const wholesaleAvailable=wholesalePrice>0&&wholesaleMinQuantity>0&&product.quantity>=wholesaleMinQuantity;
  const minimumQuantity=selectedPriceTier==="wholesale"&&wholesaleAvailable?wholesaleMinQuantity:1;
  ...
  const chooseWholesale=()=>{if(!wholesaleAvailable)return;setSelectedPriceTier("wholesale");setQuantity(current=>Math.max(current,wholesaleMinQuantity));};

  return <main className="min-h-[calc(100vh-4rem)] bg-slate-50 px-3 py-5 sm:px-6 sm:py-7 lg:px-8"><div className="mx-auto max-w-[1020px]">
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,460px)_minmax(0,520px)] lg:gap-6">
      <section className="w-full max-w-[460px]">
        <button type="button" onClick={()=>setGalleryOpen(true)} className="relative block aspect-square w-full overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 text-left" aria-label={tr("Отвори галерията","Open gallery")}>
          <img src={images[selectedImage]||"/placeholder-image.jpg"} alt={product.title} className="h-full w-full object-cover transition duration-200 hover:scale-[1.015]"/>
        </button>
        {images.length>1&&<div className="mt-3 flex gap-2.5 overflow-x-auto pb-1">{images.map((image,index)=><button key={`${image}-${index}`} type="button" onClick={()=>setSelectedImage(index)} className={`h-16 w-16 shrink-0 overflow-hidden rounded-lg border-2 bg-slate-100 transition ${selectedImage===index?"border-[#18b99f]":"border-slate-200 hover:border-slate-400"}`}><img src={image} alt={`${product.title} ${index+1}`} className="h-full w-full object-cover"/></button>)}</div>}
      </section>
      ...
      <section className="h-fit rounded-2xl border border-slate-200 bg-white p-5 lg:p-6">
        ...
        <div className="mt-3 flex">{[1,2,3,4,5].map(s=><StarIcon key={s} className={`h-4 w-4 ${s<=Math.round(rating)?"text-yellow-400":"text-slate-200"}`}/>)}</div>

        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <button type="button" onClick={()=>setSelectedPriceTier("retail")} className={`rounded-xl border bg-slate-50 p-3 text-left transition ${selectedPriceTier==="retail"?"border-[#18b99f] ring-2 ring-[#18b99f]/20":"border-slate-200 hover:border-slate-300"}`}>
            <div className="text-[11px] font-bold uppercase tracking-wide text-slate-500">{tr("Цена на дребно","Retail price")}</div>
            <div className="mt-1 text-2xl font-black text-slate-950">{formatCurrency(displayPrice)}</div>
          </button>
          <button type="button" onClick={chooseWholesale} disabled={!wholesaleAvailable} className={`rounded-xl border bg-slate-50 p-3 text-left transition ${selectedPriceTier==="wholesale"?"border-[#18b99f] ring-2 ring-[#18b99f]/20":"border-slate-200 hover:border-slate-300"} disabled:cursor-not-allowed disabled:opacity-50`}>
            <div className="text-[11px] font-bold uppercase tracking-wide text-slate-500">{tr("Цена на едро","Wholesale price")}</div>
            <div className="mt-1 text-2xl font-black text-slate-950">{wholesalePrice>0?formatCurrency(wholesalePrice):tr("Не е зададена","Not set")}</div>
            {wholesalePrice>0&&wholesaleMinQuantity>0&&<div className="mt-1 text-[11px] text-slate-500">{tr(`При минимум ${wholesaleMinQuantity} бр.`,`From ${wholesaleMinQuantity} pcs.`)}</div>}
          </button>
        </div>

        <div className="mt-4 border-y border-slate-200 py-3"><strong className={`inline-flex items-center gap-2 text-sm ${product.quantity>0?"text-emerald-700":"text-rose-600"}`}>{product.quantity>0?<><CheckCircleIcon className="h-5 w-5"/>{tr("В наличност","In stock")}</>:tr("Изчерпан продукт","Out of stock")}</strong></div>
        ...
        {product.quantity>0&&<div className="mt-5 flex items-center gap-3"><button type="button" onClick={()=>setQuantity(Math.max(minimumQuantity,quantity-1))} className="flex h-9 w-9 items-center justify-center rounded-md border border-slate-300 bg-white text-lg">−</button><input type="number" min={minimumQuantity} max={product.quantity} value={quantity} onChange={e=>setQuantity(Math.min(product.quantity,Math.max(minimumQuantity,Number(e.target.value)||minimumQuantity)))} className="h-9 w-20 rounded-md border border-slate-300 p-2 text-center"/><button type="button" onClick={()=>setQuantity(Math.min(product.quantity,quantity+1))} className="flex h-9 w-9 items-center justify-center rounded-md border border-slate-300 bg-white text-lg">+</button></div>}
        <button onClick={()=>void addToCart()} disabled={product.quantity<=0} className="mt-5 w-full rounded-xl bg-slate-950 px-5 py-3 font-bold text-white disabled:opacity-40">{product.quantity>0?tr("Добави в количката","Add to cart"):tr("Изчерпан продукт","Unavailable")}</button>
  ...
  {galleryOpen&&<div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 p-4" onClick={()=>setGalleryOpen(false)} role="dialog" aria-modal="true">
    <button type="button" onClick={e=>{e.stopPropagation();setGalleryOpen(false);}} className="absolute right-5 top-5 z-30 rounded-full bg-black/50 p-3 text-white hover:bg-black/70" aria-label={tr("Затвори","Close")}><XMarkIcon className="h-7 w-7"/></button>
    {images.length>1&&<button type="button" onClick={e=>{e.stopPropagation();previousImage();}} className="absolute left-4 top-1/2 z-30 -translate-y-1/2 rounded-full bg-black/50 p-3 text-white hover:bg-black/70" aria-label={tr("Предишна снимка","Previous image")}><ChevronLeftIcon className="h-8 w-8"/></button>}
    <img src={images[selectedImage]||"/placeholder-image.jpg"} alt={product.title} onClick={e=>e.stopPropagation()} className="max-h-[86vh] max-w-[86vw] object-contain"/>
    {images.length>1&&<button type="button" onClick={e=>{e.stopPropagation();nextImage();}} className="absolute right-4 top-1/2 z-30 -translate-y-1/2 rounded-full bg-black/50 p-3 text-white hover:bg-black/70" aria-label={tr("Следваща снимка","Next image")}><ChevronRightIcon className="h-8 w-8"/></button>}
    {images.length>1&&<div className="absolute bottom-5 left-1/2 z-30 flex max-w-[90vw] -translate-x-1/2 gap-2 overflow-x-auto rounded-2xl bg-black/55 p-2" onClick={e=>e.stopPropagation()}>{images.map((image,index)=><button key={`${image}-modal-${index}`} type="button" onClick={()=>setSelectedImage(index)} className={`h-14 w-14 shrink-0 overflow-hidden rounded-lg border-2 ${selectedImage===index?"border-[#18b99f]":"border-white/30"}`}><img src={image} alt={`${product.title} ${index+1}`} className="h-full w-full object-cover"/></button>)}</div>}
  </div>}
  </main>;
};
