...
const ProductDetails = () => {
  ...
  const addToCart=async()=>{if(!product||product.quantity<=0)return;try{dispatch(addItem({id:product.id,title:product.title,regularPrice:product.regularPrice,quantity,imageUrl:product.mainImageUrl,mainImageUrl:product.mainImageUrl,discountPercentage:product.discountPercentage,discountedPrice:product.discountedPrice}));toast.success(tr("Продуктът е добавен в количката.","Product added to cart."));if(token){void fetch(`${process.env.NEXT_PUBLIC_API_URL}/Orders`,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({productId:product.id,quantity})}).catch(()=>undefined);}}catch{toast.error(tr("Продуктът не беше добавен в количката.","The product was not added to your cart."));}};
  ...
  {galleryOpen&&<div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 p-4" onClick={()=>setGalleryOpen(false)}>
    <button type="button" onClick={()=>setGalleryOpen(false)} className="absolute right-5 top-5 rounded-full bg-white/10 p-3 text-white hover:bg-white/20" aria-label={tr("Затвори","Close")}><XMarkIcon className="h-7 w-7"/></button>
    {images.length>1&&<button type="button" onClick={e=>{e.stopPropagation();previousImage();}} className="absolute left-4 rounded-full bg-white/10 p-3 text-white hover:bg-white/20"><ChevronLeftIcon className="h-8 w-8"/></button>}
    <img src={images[selectedImage]||"/placeholder-image.jpg"} alt={product.title} onClick={e=>e.stopPropagation()} className="max-h-[82vh] max-w-[88vw] object-contain"/>
    {images.length>1&&<button type="button" onClick={e=>{e.stopPropagation();nextImage();}} className="absolute right-4 rounded-full bg-white/10 p-3 text-white hover:bg-white/20"><ChevronRightIcon className="h-8 w-8"/></button>}
    {images.length>1&&<div className="absolute bottom-5 left-1/2 flex max-w-[90vw] -translate-x-1/2 gap-2 overflow-x-auto rounded-2xl bg-black/40 p-2">{images.map((image,index)=><button key={`${image}-modal-${index}`} type="button" onClick={e=>{e.stopPropagation();setSelectedImage(index);}} className={`h-16 w-16 shrink-0 overflow-hidden rounded-lg border-2 ${selectedImage===index?"border-[#18b99f]":"border-white/30"}`}><img src={image} alt={`${product.title} ${index+1}`} className="h-full w-full object-cover"/></button>)}</div>}
  </div>}
  </main>;
};
