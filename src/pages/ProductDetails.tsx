...
const ProductDetails = () => {
  ...
  const addToCart = async () => {
    ...
    const wholesaleBase = Number(product.wholesalePriceInclVat ?? product.wholesalePrice ?? 0);
    const retailPromoActive = Number(product.discountedPrice ?? 0) > 0 && Number(product.discountedPrice) < product.regularPrice;
    const effectiveDiscount = Number(product.discountPercentage ?? 0) > 0
      ? Number(product.discountPercentage)
      : retailPromoActive && product.regularPrice > 0
        ? (1 - Number(product.discountedPrice) / product.regularPrice) * 100
        : 0;
    const wholesalePromo = effectiveDiscount > 0 ? Math.round(wholesaleBase * (1 - effectiveDiscount / 100) * 100) / 100 : wholesaleBase;
    const useWholesale = selectedPriceTier === "wholesale" && wholesaleBase > 0;

    try {
      dispatch(addItem({
        id: product.id,
        title: product.title,
        regularPrice: useWholesale ? wholesalePromo : product.regularPrice,
        quantity,
        imageUrl: product.mainImageUrl,
        mainImageUrl: product.mainImageUrl,
        discountPercentage: useWholesale ? 0 : product.discountPercentage,
        discountedPrice: useWholesale ? 0 : product.discountedPrice,
      }));
  ...
  const wholesalePromoPrice = discountPercent > 0
    ? Math.round(wholesalePrice * (1 - discountPercent / 100) * 100) / 100
    : wholesalePrice;
  ...
  const chooseWholesale = () => {
    ...
    setSelectedPriceTier("wholesale");
    setQuantity((current) => Math.max(current, wholesaleMinQuantity));
  };
    ...
    <main className="min-h-[calc(100vh-4rem)] bg-slate-50 px-3 py-5 sm:px-6 sm:py-7 lg:px-8">
        ...
        <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,460px)_minmax(0,520px)] lg:gap-6">
          ...
          <section className="h-fit rounded-2xl border border-slate-200 bg-white p-5 lg:p-6">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <button
                type="button"
                onClick={() => setSelectedPriceTier("retail")}
                className={`rounded-xl border bg-slate-50 p-3 text-left transition ${selectedPriceTier === "retail" ? "border-[#18b99f] ring-2 ring-[#18b99f]/20" : "border-slate-200 hover:border-slate-300"}`}
              >
              ...
              <button
                type="button"
                onClick={chooseWholesale}
                disabled={!wholesaleAvailable}
                className={`rounded-xl border bg-slate-50 p-3 text-left transition ${selectedPriceTier === "wholesale" ? "border-[#18b99f] ring-2 ring-[#18b99f]/20" : "border-slate-200 hover:border-slate-300"} disabled:cursor-not-allowed disabled:opacity-50`}
              >
                <div className="text-[11px] font-bold uppercase tracking-wide text-slate-500">{tr("Цена на едро", "Wholesale price")}</div>
                {discountPercent > 0 && wholesalePrice > 0 && <div className="mt-1 text-sm font-semibold text-slate-400 line-through">{formatCurrency(wholesalePrice)}</div>}
                <div className={`mt-0.5 text-2xl font-black ${discountPercent > 0 && wholesalePrice > 0 ? "text-rose-600" : "text-slate-950"}`}>{wholesalePrice > 0 ? formatCurrency(wholesalePromoPrice) : tr("Не е зададена", "Not set")}</div>
            ...
            {product.quantity > 0 && (
              <div className="mt-5 flex items-center gap-3">
                <button type="button" onClick={() => setQuantity(Math.max(minimumQuantity, quantity - 1))} className="flex h-9 w-9 items-center justify-center rounded-md border border-slate-300 bg-white text-lg">−</button>
                <input type="number" min={minimumQuantity} max={product.quantity} value={quantity} onChange={(event) => setQuantity(Math.min(product.quantity, Math.max(minimumQuantity, Number(event.target.value) || minimumQuantity)))} className="h-9 w-20 rounded-md border border-slate-300 p-2 text-center" />
        ...
        <section className="mt-8 rounded-[2rem] border border-slate-200 bg-white p-5 sm:p-7">
          ...
          {token ? (
            <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mt-3 flex items-center justify-between gap-3"><span className="text-xs text-slate-500">{tr("Ревю може да остави само профил с потвърдена поръчка за този продукт.", "Only an account with a confirmed order for this product can review it.")}</span><button type="button" disabled={reviewRating < 1 || sendingReview} onClick={() => void submitReview()} className="rounded-xl bg-[#18b99f] px-5 py-2.5 font-bold text-white disabled:opacity-40">{sendingReview ? tr("Изпращане...", "Submitting...") : tr("Публикувай", "Publish")}</button></div>
            </div>
          ...
          <div className="mt-6 space-y-3">
            {reviews.length === 0
              ...
              : reviews.map((review) => <article key={review.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-center justify-between gap-3"><strong>{review.userNames}</strong><div className="flex">{[1, 2, 3, 4, 5].map((star) => <StarIcon key={star} className={`h-4 w-4 ${star <= review.rating ? "text-yellow-400" : "text-slate-200"}`} />)}</div></div>{review.content?.trim() ? <p className="mt-2 text-sm text-slate-700">{review.content}</p> : null}</article>)}
          </div>
        </section>
      </div>
      ...
      {galleryOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 p-4" onClick={() => setGalleryOpen(false)} role="dialog" aria-modal="true">
          ...
          {images.length > 1 && <div className="absolute bottom-5 left-1/2 z-30 flex max-w-[90vw] -translate-x-1/2 gap-2 overflow-x-auto rounded-2xl bg-black/55 p-2" onClick={(event) => event.stopPropagation()}>{images.map((image, index) => <button key={`${image}-modal-${index}`} type="button" onClick={() => setSelectedImage(index)} className={`h-14 w-14 shrink-0 overflow-hidden rounded-lg border-2 ${selectedImage === index ? "border-[#18b99f]" : "border-white/30"}`}><img src={image} alt={`${product.title} ${index + 1}`} className="h-full w-full object-cover" /></button>)}</div>}
        </div>
      )}
    </main>
  );
