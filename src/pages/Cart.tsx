...
import { MINIMUM_ORDER, minimumOrderMessage, ensureResponse, refreshCheckoutItems } from "../utils/checkout";
...
const Cart=()=>{
 ...
 const handleQuantityChange=async(productId:string,newQuantity:number)=>{
  ...
  const item=cart?.items.find(i=>i.productId===productId);if(!item)return;
  setBusy(true);
  try {
   if(newQuantity>item.quantity) await refreshCheckoutItems([{...item,quantity:newQuantity}],isBg);
   if(token){
    const response=await fetch(`${process.env.NEXT_PUBLIC_API_URL}/Orders`,{method:newQuantity<item.quantity?"DELETE":"PUT",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({productId,quantity:Math.abs(newQuantity-item.quantity)})});
    await ensureResponse(response,isBg);
   }
   dispatch(updateQuantity({id:productId,quantity:newQuantity}));
   if(token)await fetchCartItems();
   toast.success(isBg?"Количеството е обновено.":"Quantity updated.");
  }catch(error){toast.error(error instanceof Error?error.message:(isBg?"Количеството не можа да бъде обновено.":"Quantity could not be updated."));}finally{setBusy(false);}
 };
 const handleRemoveItem=async(productId:string)=>{
  if(busy)return;const item=cart?.items.find(i=>i.productId===productId);if(!item)return;
  setBusy(true);
  try {
   if(token){const response=await fetch(`${process.env.NEXT_PUBLIC_API_URL}/Orders`,{method:"DELETE",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({productId,quantity:item.quantity})});await ensureResponse(response,isBg);}
   dispatch(removeItem(productId));if(token)await fetchCartItems();
   toast.success(isBg?"Продуктът е премахнат.":"Product removed.");
  }catch(error){toast.error(error instanceof Error?error.message:(isBg?"Продуктът не можа да бъде премахнат.":"Could not remove product."));}finally{setBusy(false);}
 };
 ...
 return <div className="min-h-[calc(100vh-4rem)] px-3 py-6 sm:px-6 sm:py-10 lg:px-8"><div className="mx-auto max-w-7xl"><h1 className="mb-8 text-center text-3xl font-bold">{isBg?"Количка":"Cart"}</h1><div className="overflow-hidden rounded-[2rem] bg-white shadow-xl"><div className="divide-y divide-gray-200">{cart.items.map(item=><div key={item.productId} className="grid grid-cols-[80px_minmax(0,1fr)] gap-3 p-4 sm:grid-cols-[112px_minmax(0,1fr)] sm:p-6 lg:grid-cols-[112px_minmax(0,1fr)_auto] lg:items-center"><Link to={`/products/${item.productId}`}><img src={item.primaryImageUri||"/higiqlogo.png"} alt={item.title} className="h-20 w-20 rounded-xl object-cover sm:h-28 sm:w-28"/></Link><div><Link to={`/products/${item.productId}`} className="hover:text-primary-600"><h2 className="text-sm font-medium sm:text-lg">{item.title}</h2></Link><div className="mt-2 text-primary-600">{formatCurrency(item.singlePrice)} × {item.quantity} = {formatCurrency(item.totalPrice)}</div></div><div className="col-span-2 flex items-center justify-between gap-3 lg:col-span-1"><div className="flex items-center rounded-xl border"><button onClick={()=>void handleQuantityChange(item.productId,item.quantity-1)} disabled={busy||item.quantity<=1} className="min-h-11 min-w-11">−</button><span className="px-2">{item.quantity}</span><button disabled={busy} onClick={()=>void handleQuantityChange(item.productId,item.quantity+1)} className="min-h-11 min-w-11">+</button></div><button disabled={busy} aria-label={isBg?"Премахни продукта":"Remove product"} onClick={()=>void handleRemoveItem(item.productId)} className="flex min-h-11 min-w-11 items-center justify-center rounded-full border border-rose-200 text-rose-600"><XMarkIcon className="h-5 w-5"/></button></div></div>)}</div><div className="bg-gray-50 p-6">{cart.orderTotalPrice<MINIMUM_ORDER&&<p role="status" className="mb-4 rounded-xl bg-amber-50 p-4 text-amber-900">{minimumOrderMessage(cart.orderTotalPrice,isBg)} <Link to="/products" className="underline">{isBg?"Добави продукти":"Add products"}</Link></p>}<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-xl font-semibold">{isBg?"Общо за поръчката:":"Order total:"} {formatCurrency(cart.orderTotalPrice)}</p><button disabled={busy||cart.orderTotalPrice<MINIMUM_ORDER} onClick={()=>navigate("/checkout")} className="min-h-12 rounded-md disabled:opacity-50 bg-primary-600 px-6 py-3 text-white">{token?(isBg?"Към завършване":"Continue to checkout"):(isBg?"Поръчай като гост":"Checkout as guest")}</button></div></div></div></div></div>;
};
export default Cart;
