...
const AdminSlideshow = () => {
    ...
    try {
      ...
      setSlides(
        Array.isArray(payload?.slides)
          ? [...payload.slides].sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
          : []
      );
    } catch (err) {
  ...
  if (loading) {
    return <div className="flex min-h-80 items-center justify-center"><div className="h-12 w-12 animate-spin rounded-full border-b-2 border-t-2 border-[#18b99f]" /></div>;
  }
          ...
          <section key={slide.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-col gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => moveSlide(index, -1)} disabled={index === 0} className="rounded-md border border-slate-300 bg-white p-2 text-slate-600 disabled:opacity-30" title={isBg ? "Нагоре" : "Move up"}><ArrowUpIcon className="h-5 w-5" /></button>
                <button type="button" onClick={() => moveSlide(index, 1)} disabled={index === slides.length - 1} className="rounded-md border border-slate-300 bg-white p-2 text-slate-600 disabled:opacity-30" title={isBg ? "Надолу" : "Move down"}><ArrowDownIcon className="h-5 w-5" /></button>
                <button type="button" onClick={() => deleteSlide(slide.id)} className="rounded-md bg-red-600 p-2 text-white hover:bg-red-700" title={isBg ? "Изтрий слайда" : "Delete slide"}><TrashIcon className="h-5 w-5" /></button>
              </div>
            </div>

            <div className="grid gap-5 p-4 xl:grid-cols-[minmax(300px,0.75fr)_minmax(0,1.25fr)]">
              <div className="space-y-3">
                <div className={`relative aspect-[16/7] overflow-hidden rounded-xl bg-gradient-to-r ${slide.accent || gradientOptions[0].value} ring-1 ring-slate-200`}>
                  {slide.image ? <img src={slide.image} alt={slide.titleBg || "Slide"} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-sm text-slate-400">{isBg ? "Няма снимка" : "No image"}</div>}
                </div>
                <div>
                ...
                <div>
                  ...
                  <select value={slide.accent} onChange={(event) => updateSlide(slide.id, "accent", event.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
                    {gradientOptions.map((option) => <option key={option.value} value={option.value}>{isBg ? option.bg : option.en}</option>)}
                  </select>
                </div>
              </div>
...
const TextInput = ({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) => (
  <label className="block">
    ...
    <input type="text" value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-[#18b99f] focus:outline-none focus:ring-2 focus:ring-[#18b99f]/15" />
  </label>
);
...
const TextArea = ({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) => (
  <label className="block">
    <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</span>
    <textarea rows={4} value={value} onChange={(event) => onChange(event.target.value)} className="w-full resize-y rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-[#18b99f] focus:outline-none focus:ring-2 focus:ring-[#18b99f]/15" />
  </label>
);
