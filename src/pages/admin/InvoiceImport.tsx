...
const InvoiceImport = () => {
  ...
  const allConfirmed = rows.length > 0 && rows.every((row, index) => Boolean(row.selectedProductId) && Number.isInteger(Number(row.editableQuantity)) && Number(row.editableQuantity) > 0 && confirmedQuantities.has(index));
  ...
  const resetQuantityConfirmation = (index: number) => {
    setConfirmedQuantities((current) => { const next = new Set(current); next.delete(index); return next; });
  };
  const toggleQuantity = (index: number) => {
    setConfirmedQuantities((current) => { const next = new Set(current); next.has(index) ? next.delete(index) : next.add(index); return next; });
  };
  ...
  const resetReview = () => { setResult(null); setRows([]); setConfirmedQuantities(new Set()); setInvoiceNumber(""); setReviewOpen(false); setProgress(0); setError(""); setNewProductRow(null); };
